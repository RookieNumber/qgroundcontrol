...
QGC_LOGGING_CATEGORY(SprayingComplexItemLog, "SprayingComplexItemLog")
...
SprayingComplexItem::SprayingComplexItem(PlanMasterController* masterController, bool flyView, const QString& kmlOrShpFile)
    ...
    , _metaDataMap              (FactMetaData::createMapFromJsonFile(QStringLiteral(":/json/Spraying.SettingsGroup.json"), this))
...
{
    _editorQml = "qrc:/qml/QGroundControl/Controls/SprayingItemEditor.qml";
    ...
    connect(&_gridAngleFact,            &Fact::valueChanged,                        this, &SprayingComplexItem::_setDirty);
    connect(&_flyAlternateTransectsFact,&Fact::valueChanged,                        this, &SprayingComplexItem::_setDirty);
    connect(&_splitConcavePolygonsFact, &Fact::valueChanged,                        this, &SprayingComplexItem::_setDirty);
    connect(this,                       &SprayingComplexItem::refly90DegreesChanged,  this, &SprayingComplexItem::_setDirty);

    connect(&_gridAngleFact,            &Fact::valueChanged,                        this, &SprayingComplexItem::_rebuildTransects);
    connect(&_flyAlternateTransectsFact,&Fact::valueChanged,                        this, &SprayingComplexItem::_rebuildTransects);
    connect(&_splitConcavePolygonsFact, &Fact::valueChanged,                        this, &SprayingComplexItem::_rebuildTransects);
    connect(this,                       &SprayingComplexItem::refly90DegreesChanged,  this, &SprayingComplexItem::_rebuildTransects);

    connect(&_surveyAreaPolygon,        &QGCMapPolygon::isValidChanged,             this, &SprayingComplexItem::_updateWizardMode);
    connect(&_surveyAreaPolygon,        &QGCMapPolygon::traceModeChanged,           this, &SprayingComplexItem::_updateWizardMode);
...
void SprayingComplexItem::save(QJsonArray&  planItems)
{
...
void SprayingComplexItem::savePreset(const QString& name)
{
    QJsonObject saveObject;
...
void SprayingComplexItem::_saveCommon(QJsonObject& saveObject)
{
...
void SprayingComplexItem::loadPreset(const QString& name)
{
    QString errorString;
...
bool SprayingComplexItem::load(const QJsonObject& complexObject, int sequenceNumber, QString& errorString)
{
...
bool SprayingComplexItem::_loadV4V5(const QJsonObject& complexObject, int sequenceNumber, QString& errorString, int version, bool forPresets)
{
...
bool SprayingComplexItem::_loadV3(const QJsonObject& complexObject, int sequenceNumber, QString& errorString)
{
...
void SprayingComplexItem::_reverseTransectOrder(QList<QList<QGeoCoordinate>>& transects)
{
...
void SprayingComplexItem::_reverseInternalTransectPoints(QList<QList<QGeoCoordinate>>& transects)
{
...
void SprayingComplexItem::_optimizeTransectsForShortestDistance(const QGeoCoordinate& distanceCoord, QList<QList<QGeoCoordinate>>& transects)
{
...
qreal SprayingComplexItem::_ccw(QPointF pt1, QPointF pt2, QPointF pt3)
{
    return (pt2.x()-pt1.x())*(pt3.y()-pt1.y()) - (pt2.y()-pt1.y())*(pt3.x()-pt1.x());
}

qreal SprayingComplexItem::_dp(QPointF pt1, QPointF pt2)
{
    return (pt2.x()-pt1.x())/qSqrt((pt2.x()-pt1.x())*(pt2.x()-pt1.x()) + (pt2.y()-pt1.y())*(pt2.y()-pt1.y()));
}

void SprayingComplexItem::_swapPoints(QList<QPointF>& points, int index1, int index2)
{
...
bool SprayingComplexItem::_gridAngleIsNorthSouthTransects()
{
...
void SprayingComplexItem::_adjustTransectsToEntryPointLocation(QList<QList<QGeoCoordinate>>& transects)
{
    ...
    if (reversePoints) {
        qCDebug(SprayingComplexItemLog) << "_adjustTransectsToEntryPointLocation Reverse Points";
        _reverseInternalTransectPoints(transects);
    }
    if (reverseTransects) {
        qCDebug(SprayingComplexItemLog) << "_adjustTransectsToEntryPointLocation Reverse Transects";
        _reverseTransectOrder(transects);
    }

    qCDebug(SprayingComplexItemLog) << "_adjustTransectsToEntryPointLocation Modified entry point:entryLocation" << transects.first().first() << _entryPoint;
}

QPointF SprayingComplexItem::_rotatePoint(const QPointF& point, const QPointF& origin, double angle)
{
    QPointF rotated;
...
void SprayingComplexItem::_intersectLinesWithRect(const QList<QLineF>& lineList, const QRectF& boundRect, QList<QLineF>& resultLines)
{
...
void SprayingComplexItem::_intersectLinesWithPolygon(const QList<QLineF>& lineList, const QPolygonF& polygon, QList<QLineF>& resultLines)
{
    resultLines.clear();
...
void SprayingComplexItem::_adjustLineDirection(const QList<QLineF>& lineList, QList<QLineF>& resultLines)
{
    qreal firstAngle = 0;
    for (int i=0; i<lineList.count(); i++) {
        const QLineF& line = lineList[i];
...
double SprayingComplexItem::_clampGridAngle90(double gridAngle)
{
...
bool SprayingComplexItem::_nextTransectCoord(const QList<QGeoCoordinate>& transectPoints, int pointIndex, QGeoCoordinate& coord)
{
...
bool SprayingComplexItem::_hasTurnaround(void) const
{
    return _turnAroundDistance() > 0;
}

double SprayingComplexItem::_turnaroundDistance(void) const
{
    return _turnAroundDistanceFact.rawValue().toDouble();
}

void SprayingComplexItem::_rebuildTransectsPhase1(void)
{
...
void SprayingComplexItem::_rebuildTransectsPhase1WorkerSinglePolygon(bool refly)
{
    if (_ignoreRecalc) {
        return;
    }
    ...
    qCDebug(SprayingComplexItemLog) << "_rebuildTransectsPhase1 Convert polygon to NED - _surveyAreaPolygon.count():tangentOrigin" << _surveyAreaPolygon.count() << tangentOrigin;
    for (int i=0; i<_surveyAreaPolygon.count(); i++) {
        ...
        qCDebug(SprayingComplexItemLog) << "_rebuildTransectsPhase1 vertex:x:y" << vertex << polygonPoints.last().x() << polygonPoints.last().y();
    }
    ...
    qCDebug(SprayingComplexItemLog) << "_rebuildTransectsPhase1 Clamped grid angle" << gridAngle;

    qCDebug(SprayingComplexItemLog) << "_rebuildTransectsPhase1 gridSpacing:gridAngle:refly" << gridSpacing << gridAngle << refly;

    // Convert polygon to bounding rect

    qCDebug(SprayingComplexItemLog) << "_rebuildTransectsPhase1 Polygon";
    QPolygonF polygon;
    for (int i=0; i<polygonPoints.count(); i++) {
        qCDebug(SprayingComplexItemLog) << "Vertex" << polygonPoints[i];
        polygon << polygonPoints[i];
    }
    polygon << polygonPoints[0];
    QRectF boundingRect = polygon.boundingRect();
    QPointF boundingCenter = boundingRect.center();
    qCDebug(SprayingComplexItemLog) << "Bounding rect" << boundingRect.topLeft().x() << boundingRect.topLeft().y() << boundingRect.bottomRight().x() << boundingRect.bottomRight().y();
    ...
    for (const QList<QGeoCoordinate>& transect : transects) {
        ...
        if (triggerCamera() && hoverAndCaptureEnabled()) {
            ...
            if (triggerDistance() < transectLength) {
                ...
                qCDebug(SprayingComplexItemLog) << "cInnerHoverPoints" << cInnerHoverPoints;
...
void SprayingComplexItem::_rebuildTransectsPhase1WorkerSplitPolygons(bool refly)
{
    if (_ignoreRecalc) {
        return;
    }
...
//        qCDebug(SprayingComplexItemLog) << "Transects from polynom p " << p;
        _rebuildTransectsFromPolygon(refly, *p, tangentOrigin, vMatch);
    }
}

void SprayingComplexItem::_PolygonDecomposeConvex(const QPolygonF& polygon, QList<QPolygonF>& decomposedPolygons)
{
...
bool SprayingComplexItem::_VertexCanSeeOther(const QPolygonF& polygon, const QPointF* vertexA, const QPointF* vertexB) {
...
//    qCDebug(SprayingComplexItemLog) << "_VertexCanSeeOther false after first checks ";
...
//    qCDebug(SprayingComplexItemLog) << "_VertexCanSeeOther distanceAB " << distanceAB;
...
//            qCDebug(SprayingComplexItemLog) << "*vertexA " << *vertexA << "*vertexB " << *vertexB  << " intersection " << intersection;
            ...
            qCDebug(SprayingComplexItemLog) << "_VertexCanSeeOther distanceIntersection " << distanceIntersection;
...
bool SprayingComplexItem::_VertexIsReflex(const QPolygonF& polygon, QList<QPointF>::const_iterator& vertexIter) {
    auto vertexBefore = vertex == polygon.begin() ? polygon.end() - 1 : vertex - 1;
    auto vertexAfter = vertex == polygon.end() - 1 ? polygon.begin() : vertex + 1;
...
void SprayingComplexItem::_rebuildTransectsFromPolygon(bool refly, const QPolygonF& polygon, const QGeoCoordinate& tangentOrigin, const QPointF* const transitionPoint)
{
    // Generate transects
    ...
    qCDebug(SprayingComplexItemLog) << "_transects.size() " << _transects.size();
}

void SprayingComplexItem::_recalcCameraShots(void)
{
...
SprayingComplexItem::ReadyForSaveState SprayingComplexItem::readyForSaveState(void) const
{
    return TransectStyleComplexItem::readyForSaveState();
}

void SprayingComplexItem::rotateEntryPoint(void)
{
...
double SprayingComplexItem::timeBetweenShots(void)
{
    return _vehicleSpeed == 0 ? 0 : triggerDistance() / _vehicleSpeed;
}

double SprayingComplexItem::additionalTimeDelay (void) const
{
    double hoverTime = 0;
...
void SprayingComplexItem::_updateWizardMode(void)
{
